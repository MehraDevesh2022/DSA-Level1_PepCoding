...
public class Main {
    ...
    public static class LinkedList {
        ...
        public int mid() {
           // create pointer slow of fast where both are pointing to head of linklist
           Node slow = head;
           Node fast =head;
           // when list is empty
           if(head==null)
           {
               return head.data; // return type is int therefore data returns
           }
           while(fast.next !=null && fast.next.next !=null) // condtion checks for odd or even both list
           {
                  slow=slow.next; // slow is trevel one step and fast is 2x of slow . when fast is arived in tail then slow is arived mid of linklist
                  fast=fast.next.next;
           }
           return slow.data; // we need mid so return mid
