...
public class Main {
  ...
  public static class LinkedList {
    ...
    public int kthFromLast(int k) {
      ...
      // without using size property we need to return kth node data from last to
      // given kth index.
      Node fast = head;
      Node slow = head;
      // first we travel fast to kth from start
      for (int i = 0; i < k; i++) {
        fast = fast.next;

      }
      while (fast.next != null) {
        slow = slow.next; // then we move both where slow is in head
        fast = fast.next; // fast is in kth position and travel untill arive to tail

      }
      return slow.data; // when fast.next is arived to null so slow is in k th position
    }
  }
