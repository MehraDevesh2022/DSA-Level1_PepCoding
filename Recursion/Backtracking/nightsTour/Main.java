...
public class Main {
    ...
    public static void main(String[] args) throws Exception {
        ...
        int row = scn.nextInt();
        int clm = scn.nextInt();
        printKnightsTour(chess, row, clm, 1);
    }

    public static void printKnightsTour(int[][] chess, int r, int c, int upcomingMove) {
         // this check is for if checking  row and clm limit after call(ghde main girke)
        // if (r < 0 || c < 0 || r >= chess.length || c >= chess.length || chess[r][c] != 0)
        //     return;

        // base case
        if (upcomingMove == chess.length * chess.length) {
            chess[r][c] = upcomingMove; // mark last place
            displayBoard(chess);
            chess[r][c] = 0; // unmark last val
            return;
        }
        // mark number of step in block
        chess[r][c] = upcomingMove;
        for (int d = 0; d < dir.length; d++) {
            int row = r + dir[d][0];
            int clm = c + dir[d][1];
            // this check is when we are under limit (check  ghde main girne  se pehle)
             if(row >=0 && clm >=0 && row<chess.length && clm<chess.length && chess[row][clm] ==0)
             printKnightsTour(chess, row, clm, upcomingMove + 1);
